...
function PlayHistoryClass() {
			...
			$.get(Root +"index.php?s=User-Comm-getplaylog", function(result){												 
				if(result['rcode']>-1){
					...
					if(result['r']!=null&&result['r'].length>0){
						...
						document.getElementById('playhistory').innerHTML="<li class='no-his'>暂无记录</li>";
					}
				}
			},'json');
			$("#his-todo").hide(); 
				...
				for ( var i = nameArray.length - 1; i >= 0; i--) {
					...
					if (vodnames.length == 2 && vodlinks.length ==2) {
						innerStr += "<li><h5><a href=\""
								...
								+ "\">"
								+ vodnames[0]
								+ "</a><em>/</em><a class=\"zt\" target='_blank' href=\"" 
								+ vodlinks[1]
								+"\">"
								+ vodlinks[0]
	...
	this.emptyhistory = function(type){
		$.showfloatdiv({
			...
			suredo : function(e) {
				if(type=='ajax'){
					$.get(Root + 'index.php?s=User-Comm-emptyhistory',function(r){document.getElementById('playhistory').innerHTML="<li class='no-his'>暂无记录</li>";});
				}else{
					_GC();
				}
				$.closefloatdiv();
